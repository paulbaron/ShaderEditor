...
{
    QMatrix4x4 tmp(glm::value_ptr(_matrix));

    program->setUniformValue(inputName.toStdString().c_str(), tmp.transposed());
    return (0);
}
...
//
            _matrix = static_cast<MatPerspectiveView*>(matView->getCurrent())->getPerspective();
        }
        break;
    default:
        break;
